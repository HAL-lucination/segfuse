...
class DeConv(Decoder):
    def __init__(self, in_channels, kernel_size):
        ...
        def convt(in_channels):
            ...
            # DeConv module: 2*2 conv -> batchnorm -> ReLU
        ...
        self.layer3 = convt(in_channels // 4)
        self.layer4 = convt(in_channels // 8)
...
class MyModel(nn.Module):
    def __init__(self, layers, decoder, output_size=None, in_channels=3, pretrained=True, prediction_size = None, training_stage = 1):
        ...
        self.equi_conv4 = nn.Sequential(
                nn.Conv2d(num_channels//32, 150, kernel_size=3, stride=1, padding=1, bias=False),
                nn.UpsamplingBilinear2d(size=self.prediction_size)
                )
        self.log_softmax = nn.LogSoftmax(dim=None) ## here added log_softmax
    ...
    def forward_FCRN_fusion(self, equi, fusion=False):
        cube = self.ce.E2C(equi) # convert equi to cube surfaces
        feat_equi = self.equi_model.pre_encoder2(equi) # The first equi conv layer replaced with Pre-Block
        feat_cube = self.cube_model.pre_encoder(cube) # The first cube conv layer

        # ENCODER SECTION

        for e in range(5): # Fusing equi and cube for 5 times when downsample
            if fusion:
                # Fusion block between equi and cube
        ...
        # Shared parts
        for d in range(shared_layers): # Fusing equi and cube for x times before branching
            if fusion:
                aaa = self.conv_e2c[d+5](feat_equi) # start from 6th
                tmp_cube = self.ce.E2C(aaa)
            ...
            # break point
            # print(self.equi_decoder)

            feat_equi = getattr(self.equi_decoder, 'layer%d'%(d+1))(feat_equi)
            feat_cube = getattr(self.cube_decoder, 'layer%d'%(d+1))(feat_cube)

        feat_equi_seg = feat_equi # shared equi latent space for multitasking
        feat_cube_seg = feat_cube # shared cube latent space for multitasking

        # Depth Estimation branch
        # for d in range(4 - shared_layers): # Fusing equi and cube for x times before branching
        #     if fusion:
        #         aaa = self.conv_e2c[d+5](feat_equi) # start from 6th
        #         tmp_cube = self.ce.E2C(aaa)
        #         tmp_equi = self.conv_c2e[d+5](self.ce.C2E(feat_cube))
        ...
        equi_depth = self.equi_conv3(feat_equi) # 3x3 conv with up-sampling to target resolution
        cube_depth = self.cube_conv3(feat_cube) # padding, 3x3 conv with up-sampling to target resolution
        ...
        # for d in range(4 - shared_layers):
        #     feat_equi_seg = getattr(self.equi_decoder, 'layer%d'%(d+1))(feat_equi_seg)
        #     # feat_cube_seg = getattr(self.cube_decoder, 'layer%d'%(d+1))(feat_cube_seg)

        equi_seg = self.equi_conv4(feat_equi_seg) # 3x3 conv with up-sampling to target resolution
        # equi_seg = self.log_softmax(equi_seg)
        # cube_seg = self.cube_conv3(feat_cube_seg) # padding, 3x3 conv with up-sampling to target resolution

        return equi_depth, cube_depth, refine_final, equi_seg
